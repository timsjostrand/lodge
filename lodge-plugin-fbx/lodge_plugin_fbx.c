...
static struct lodge_ret lodge_fbx_plugin_init(struct lodge_res *fbx_res, struct lodge_plugins *plugins)
{
...
static void lodge_fbx_plugin_free(struct lodge_res *fbx_res)
{
	lodge_res_free_inplace(fbx_res);
}

struct lodge_plugin lodge_fbx_plugin()
{
	return (struct lodge_plugin) {
		.version = LODGE_PLUGIN_VERSION,
		.size = lodge_res_sizeof(),
		.name = strview_static("fbx"),
		.init = &lodge_fbx_plugin_init,
		.free = &lodge_fbx_plugin_free,
		.update = NULL,
		.render = NULL,
	};
}
