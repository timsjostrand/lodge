...
static inline vec3 vec4_clip_to_ndc_space(const vec4 clip_space)
{
	return vec3_div_scalar(vec3_make(xyz_of(clip_space)), clip_space.w);
}

static inline vec3 vec3_ndc_to_world_space(const vec3 ndc_space, const mat4 *inv_view_projection)
{
...
//
static inline vec3 vec2_screen_01_to_world_space(const vec2 screen_space, const mat4 *inv_view_projection)
{
...
// FIXME(TS): not tested
static inline vec3 vec3_world_to_view_space(const vec3 world_space, const mat4 *view)
{
...
// FIXME(TS): not tested
static inline vec3 vec3_world_to_ndc_space(const vec3 world_space, const mat4 *view_projection)
{
...
// FIXME(TS): not tested
static inline bool vec3_is_in_clip(const vec3 clip_space)
{
