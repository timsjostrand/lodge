...
struct lodge_scene_render_system
{
	...
	lodge_shader_t						shader_deferred_light;
	...
	lodge_shader_t						volumetric_light_shader;
...
{
#if 1
	lodge_gfx_bind_shader(system->shader_deferred_light);
	...
	lodge_shader_bind_constant_buffer(system->shader_deferred_light, 1, system->shadow_map.buffer_object);

	//
	// globals
	//
	lodge_shader_set_constant_vec3(system->shader_deferred_light, strview_static("sun_dir"), vec3_make(xyz_of(system->lights.elements[0].pos_dir)));

	//
	// postprocess
	//
	lodge_shader_set_constant_mvp(system->shader_deferred_light, &mvp);

	//
	// distance_fog
	//
	lodge_shader_bind_constant_buffer(system->shader_deferred_light, 2, system->distance_fog_buffer);
	lodge_shader_bind_constant_buffer(system->shader_deferred_light, 4, system->lights_buffer_object);
#else
	lodge_gfx_set_bindings(&(struct lodge_gfx_bindings_desc) {
		.shader = system->shader_deferred_light,
		.textures = {
			.count = 5,
			.elements = {
				[0] = {
					.target = LODGE_TEXTURE_TARGET_2D,
					.texture = system->geometry_buffer.albedo,
					.sampler = system->sampler_linear_clamp,
				},
				[1] = {
					.target = LODGE_TEXTURE_TARGET_2D,
					.texture = system->geometry_buffer.normals,
					.sampler = system->sampler_linear_clamp,
				},
				[2] = {
					.target = LODGE_TEXTURE_TARGET_2D,
					.texture = system->geometry_buffer.depth,
					.sampler = system->sampler_linear_clamp,
				},
				[4] = {
					.target = LODGE_TEXTURE_TARGET_2D_ARRAY,
					.texture = system->shadow_map.depth_textures_array,
					.sampler = system->sampler_linear_clamp,
				},
			}
		},
		.uniform_buffers = {
			...
			.elements = {
				[0] = {
					.buffer_object = pass_params->camera_buffer,
					.offset = 0,
					.size = 0,
				},
				[1] = {
					.buffer_object = pass_params->distance_fog_buffer,
					.offset = 0,
					.size = 0,
				},
			}
...
{
	lodge_gfx_annotate_begin(strview_static("static_meshes"));
	...
	for(size_t i = 0, count = system->count; i < count; i++) {
		...
		if(pass_params->pass == LODGE_SCENE_RENDER_SYSTEM_PASS_ENTITY_ID) {
			shader = component->shader_entity_id;
		} else {
		...
		lodge_shader_set_constant_float(shader, strview_static("entity_id"), (float)((uintptr_t)system->ids[i]));
		lodge_shader_set_constant_float(shader, strview_static("selected"), (float)((uintptr_t)system->selected[i]));
		...
		//
		//lodge_shader_set_constant_mat4(static_mesh->shader, strview_static("model"), system->static_mesh_transforms[i].model);
#endif
...
static void lodge_static_meshes_new_inplace(struct lodge_static_meshes *static_meshes, lodge_scene_t scene)
{
...
static void lodge_scene_render_system_new_inplace(struct lodge_scene_render_system *system, lodge_scene_t scene)
{
	...
	//
	{
		...
		system->volumetric_light_shader = (lodge_shader_t)lodge_assets_get(plugin->shaders, strview_static("volumetric_light"));
	}
	...
	//
	{
		...
		//
		{
			system->shader_deferred_light = (lodge_shader_t)lodge_assets_get(plugin->shaders, strview_static("deferred_light"));
	...
	lodge_static_meshes_new_inplace(&system->static_meshes, scene);
}
...
static void lodge_scene_render_system_render(struct lodge_scene_render_system *system, lodge_scene_t scene, struct lodge_system_render_params *render_params)
{
	...
	lodge_gfx_annotate_begin(strview_static("scene_render_system"));
	...
	//
	{
		lodge_gfx_annotate_begin(strview_static("shadow pass"));
		for(uint32_t cascade_index = 0; cascade_index < 3; cascade_index++) {
	...
	//
	{
		lodge_gfx_annotate_begin(strview_static("deferred pass"));
		struct lodge_scene_render_pass_params pass_params = {
	...
	//
	{
		lodge_gfx_annotate_begin(strview_static("volumetric light"));
		lodge_gfx_bind_shader(system->volumetric_light_shader);
		lodge_shader_bind_constant_buffer(system->volumetric_light_shader, 0, system->camera_buffer);
		lodge_shader_bind_constant_buffer(system->volumetric_light_shader, 1, system->shadow_map.buffer_object);
		lodge_shader_bind_constant_buffer(system->volumetric_light_shader, 2, system->distance_fog_buffer);
		lodge_shader_set_constant_vec3(system->volumetric_light_shader, strview_static("sun_dir"), lodge_directional_light_get_dir(scene));
		lodge_gfx_bind_texture_unit_2d_array(4, system->shadow_map.depth_textures_array, system->sampler_linear_clamp);
	...
	//
	{
		lodge_gfx_annotate_begin(strview_static("entity_ids"));
		struct lodge_scene_render_pass_params pass_params = {
	...
	//
	if(system->draw_post_process) {
		lodge_gfx_annotate_begin(strview_static("post process pass"));
		struct lodge_scene_render_pass_params pass_params = {
	...
	//
	{
		lodge_gfx_annotate_begin(strview_static("deferred to screen"));
	...
	//
	{
		lodge_gfx_annotate_begin(strview_static("forward-transparent pass"));
		struct lodge_scene_render_pass_params pass_params = {
...
#define lodge_type_entity_ref LODGE_TYPE_U64
		...
		LODGE_SYSTEM_TYPE_SCENE_RENDER = lodge_system_type_register((struct lodge_system_type_desc) {
			.name = strview_static("scene_renderer"),
			...
			.properties = {
				...
				.elements = {
					...
					{
						.name = strview_static("wireframe"),
						.type = LODGE_TYPE_BOOL,
						.offset = offsetof(struct lodge_scene_render_system, wireframe),
						.flags = LODGE_PROPERTY_FLAG_NONE,
					},
					{
						.name = strview_static("render_width"),
						.type = LODGE_TYPE_U32,
					...
					{
						.name = strview_static("render_height"),
						.type = LODGE_TYPE_U32,
					...
					{
						.name = strview_static("window_is_render_size"),
						.type = LODGE_TYPE_BOOL,
						.offset = offsetof(struct lodge_scene_render_system, window_is_render_size),
						.on_modified = on_modified_render_size,
					},
					{
						.name = strview_static("window_width"),
						.type = LODGE_TYPE_U32,
